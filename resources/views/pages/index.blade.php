{{-- Master Template --}}
{{-- Integrated from Project 2 to Project 3. --}}
{{-- Created By: Marc-Eli Faldas --}}
{{-- Last Modified: 4/2/2018  --}}

@extends('layouts.master')

@section('content')

    <form method='GET' action='/calculation'>
        <div class='row' id='inputSection'>

            <div class='col-sm-6'>
                <label>*Split:

                    <input type='text'
                           name='split'
                           class='splitTextBox'
                           value='{{ old('split', $split) }}'
                           placeholder='4'
                           required>
                    @include('modules.error-field', ['field' => 'split'])
                </label>

                <label>*Bill: $
                    <input type='text'
                           name='bill'
                           class='billTextBox'
                           value='{{ old('bill', $bill) }}'
                           placeholder='62.51'
                           required>
                    @include('modules.error-field', ['field' => 'bill'])
                </label>

                <p>
                    <small><em>*Required Inputs</em></small>
                </p>

            </div>

            <div class='col-sm-3'>
                <label class='tipLabel'>Tip:
                    <select name='tip' class='tipDropdown'>
                        <option value='1' {{ old('tip', $tip) == '1' ? 'selected' : '' }}>No Tip</option>
                        <option value='1.10' {{ old('tip', $tip) == '1.10' ? 'selected' : ''}}>10% Tip</option>
                        <option value='1.15' {{ old('tip', $tip) == '1.15' ? 'selected' : '' }}>15% Tip</option>
                        <option value='1.20' {{ old('tip', $tip) == '1.20' ? 'selected' : '' }}>20% Tip</option>
                    </select>
                </label>
            </div>

            <div class="col-sm-3">
                <label>Round Up:
                    <input type='checkbox'
                           name='roundUp'
                        {{ old('roundUp', $roundUp)  ? 'checked' : ''}} >
                </label>
            </div>
        </div>


        <input type='submit' value='Split It Girl!' class='splitButton' name='submit'>


    </form>


    @if($calcError)
        <div class='alert alert-danger' id='calculationError'>
            Unable to make calculation as the split bill would be less than $0.01.
        </div>
    @elseif($results)
        <div class='alert alert-success' id='calculationSuccess'>
            {{ $results }}

        </div>

        {{--Displays the welcome page as the base standard.--}}
    @elseif(count($errors) == 0)
        <div class='standard' id='welcomeMessage'>
            Hi! I'm Katy Perry. You probably don't recognize me without my blue wig. When I'm not singing on tour, making music videos or brushing Nugget's cute curls, I split bills! It's definitely a fun hobby and a great way for me to practice my math skills. Make sure to fill in the above fields. If you don't want any change, just check the "Round Up" selection and I'll round your payment to the next whole dollar. Thanks!
        </div>
    @endif

    @include('modules.error-form')

@endsection

